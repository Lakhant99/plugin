<!DOCTYPE html>
   ...
   <link rel="stylesheet" type="text/css" href="<?php echo PLUGIN_URL."/Reinforce/assets/css/sweet-alert.css" ?>" />
    ...
    <h1>Registration Form For Industry</h1>  
    <form id="wp_signup_form" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">  
    
            <label for="username">Username</label>  
            <input type="text" name="username" id="username" placeholder="ex.abc@gmail" required=""> <br> 
            <label for="email">Email address</label>  
            <input type="text" name="email" id="email" placeholder="ex.abc@gmail.com" required="">  <br>
            <label for="password">Password</label>  
            <input type="password" name="password" id="password" placeholder="Password Minimum 6 characters "required=""> <br> 
            ...
            if(0 == count($errors)) 
             {  
                ...
                //$new_user_id = wp_create_user( $username, $password, $email );
                 $user_id = wp_insert_user( array ('user_login' => apply_filters('pre_user_user_login', $username),'user_pass' => apply_filters('pre_user_user_pass', $password),'user_email' => apply_filters('pre_user_user_email', $email),'role' => 'subscriber'));
